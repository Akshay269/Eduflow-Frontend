...
export const courses = {
  ...
  search: (params: {
    ...
    return request<unknown>(`/api/courses/search?${q.toString()}`);
  },
  create: (data: any) =>
  ...
  uploadThumbnail: (id: number, file: File) => {
    ...
    fd.append("file", file);
    return request<any>(`/api/courses/${id}/thumbnail`, {
      method: "POST",
      body: fd,
    });
  },
...
export const enrollments = {
  enroll: (courseId: number) =>
    request<any>(`/api/enrollments/course/${courseId}`, { method: "POST" }),
  myEnrollments: () => request<any>("/api/enrollments/my-enrollments"),
  check: (courseId: number) =>
    request<boolean>(`/api/enrollments/check/${courseId}`),
  students: (courseId: number) =>
    request<any>(`/api/enrollments/course/${courseId}/students`),
};
...
export const reviews = {
  ...
  create: (courseId: number, data: { rating: number; comment: string }) =>
    request<any>(`/api/reviews/course/${courseId}`, {
      method: "POST",
      body: JSON.stringify(data),
    }),
  update: (reviewId: number, data: { rating: number; comment: string }) =>
    request<any>(`/api/reviews/${reviewId}`, {
      method: "PUT",
      body: JSON.stringify(data),
    }),
  delete: (reviewId: number) =>
    request<any>(`/api/reviews/${reviewId}`, { method: "DELETE" }),
  myReview: (courseId: number) =>
    request<any>(`/api/reviews/course/${courseId}/my-review`),
};
...
export const users = {
  ...
  uploadPicture: (file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    return request<any>("/api/users/profile/picture", {
      method: "POST",
      body: fd,
    });
  },
